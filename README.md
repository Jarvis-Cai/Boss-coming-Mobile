...
3. 测试UDP：
    ![发送UDP测试](pic/3.jpg)
    在完成输入IP地址之后点击 “Test” ，IP地址的获取以及测试结果报告可见同一项目工程 “Boss-coming_computer” (暂未上传)。
4.  检测开始：
    ![检测界面](pic/4.png)
    测试连接成功后，点击 “Scan” 开启检测。当检测到目标（Boss）时，将会发送警报信息到PC机当中，警报结果可见同一项目工程 “Boss-coming_computer” (暂未上传)。在开始检测过程中，为了保持隐秘性，可以点击 “Test” 按钮，将屏幕进行遮掩，遮掩效果如下图，此时在遮掩界面中点击遮眼布可以去除遮掩布。
    ![遮掩效果](pic/5.jpg)
